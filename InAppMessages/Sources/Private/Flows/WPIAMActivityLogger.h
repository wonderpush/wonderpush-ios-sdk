...
/// Values for different IAM activity types.
typedef NS_ENUM(NSInteger, WPIAMActivityType) {
    WPIAMActivityTypeFetchMessage = 0,
    WPIAMActivityTypeRenderMessage = 1,
    WPIAMActivityTypeDismissMessage = 2,
    
    // Triggered checks
    WPIAMActivityTypeCheckForOnOpenMessage = 3,
    WPIAMActivityTypeCheckForWonderPushEventMessage = 4,
    WPIAMActivityTypeCheckForFetch = 5,
};

NS_ASSUME_NONNULL_BEGIN
@interface WPIAMActivityRecord : NSObject <NSCoding>
@property(nonatomic, nonnull, readonly) NSDate *timestamp;
@property(nonatomic, readonly) WPIAMActivityType activityType;
...
/**
 * This is the class for tracking IAM flow related activity logs. Its content can later on be
 * retrieved for debugging/reporting purpose.
 */
@interface WPIAMActivityLogger : NSObject
