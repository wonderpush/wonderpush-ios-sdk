...
@implementation WPIAMActivityRecord
...
- (instancetype)initWithActivityType:(WPIAMActivityType)type
...
- (NSString *)displayStringForActivityType {
    switch (self.activityType) {
        case WPIAMActivityTypeFetchMessage:
            return @"Message Fetching";
        case WPIAMActivityTypeRenderMessage:
            return @"Message Rendering";
        case WPIAMActivityTypeDismissMessage:
            return @"Message Dismiss";
        case WPIAMActivityTypeCheckForOnOpenMessage:
            return @"OnOpen Msg Check";
        case WPIAMActivityTypeCheckForWonderPushEventMessage:
            return @"Analytic Msg Check";
        case WPIAMActivityTypeCheckForFetch:
            return @"Fetch Check";
    }
}
@end

@interface WPIAMActivityLogger ()
@property(nonatomic) BOOL isDirty;

// always insert at the head of this array so that they are always in anti-chronological order
@property(nonatomic, nonnull) NSMutableArray<WPIAMActivityRecord *> *activityRecords;
...
@implementation WPIAMActivityLogger
...
+ (NSString *)determineCacheFilePath {
    ...
    dispatch_once(&onceToken, ^{
        ...
        logCachePath = [NSString stringWithFormat:@"%@/wonderpush-iam-activity-log-cache", cacheDirPath];
        WPLogDebug(
...
- (void)loadFromCachePath:(NSString *)cacheFilePath {
    ...
    if (fetchedActivityRecords) {
        @synchronized(self) {
            self.activityRecords = (NSMutableArray<WPIAMActivityRecord *> *)fetchedActivityRecords;
            self.isDirty = NO;
        }
    }
}
...
// non verbose type.
+ (BOOL)isMandatoryType:(WPIAMActivityType)type {
    switch (type) {
        case WPIAMActivityTypeFetchMessage:
        case WPIAMActivityTypeRenderMessage:
        case WPIAMActivityTypeDismissMessage:
...
- (void)addLogRecord:(WPIAMActivityRecord *)newRecord {
    if (self.verboseMode || [WPIAMActivityLogger isMandatoryType:newRecord.activityType]) {
        @synchronized(self) {
...
- (NSArray<WPIAMActivityRecord *> *)readRecords {
