...
- (void)setUp {
    ...
    [defaults removeObjectForKey:WPRemoteConfigManager.minimumConfigAgeUserDefaultsKey];
    [defaults removeObjectForKey:WPRemoteConfigManager.maximumConfigAgeUserDefaultsKey];
...
/**
 Ensures the minimum and maximum config ages can be set by the remote config
 */
- (void) testConfigurableCache {
    ...
    // Hardcoded default values should be used at this stage
    XCTAssertEqual(self.manager.maximumConfigAge, WP_REMOTE_CONFIG_DEFAULT_MAXIMUM_CONFIG_AGE);
    XCTAssertEqual(self.manager.minimumConfigAge, WP_REMOTE_CONFIG_DEFAULT_MINIMUM_CONFIG_AGE);

    // Get the config with overridden values
    [self.manager read:^(WPRemoteConfig *config, NSError *error) {
        XCTAssertNil(error);
        XCTAssertEqual(self.manager.minimumConfigAge, 234.567);
        XCTAssertEqual(self.manager.maximumConfigAge, 123.456);

        // Create a new manager, default values should be the overridden values as well
        WPRemoteConfigManager *manager = [[WPRemoteConfigManager alloc] initWithRemoteConfigFetcher:self.fetcher storage:self.storage];
        XCTAssertEqual(manager.minimumConfigAge, 234.567);
        XCTAssertEqual(manager.maximumConfigAge, 123.456);
...
/**
 Checks that bad values in the cache settings do not affect the manager
 */

- (void) testConfigurableCacheBadValues {
    self.fetcher.fetchedConfig = [[WPRemoteConfig alloc] initWithData:@{
        @"cacheTtl" : @"hello",
        @"cacheMinAge" : @"world",
    } version:@"1.0.0"];
    ...
    // Get the config with overridden (bad) values
    [self.manager read:^(WPRemoteConfig *config, NSError *error) {

        // No error should be thrown
        XCTAssertNil(error);

        // Hardcoded default values should still be used
        XCTAssertEqual(self.manager.maximumConfigAge, WP_REMOTE_CONFIG_DEFAULT_MAXIMUM_CONFIG_AGE);
        XCTAssertEqual(self.manager.minimumConfigAge, WP_REMOTE_CONFIG_DEFAULT_MINIMUM_CONFIG_AGE);
        [expectation fulfill];
    }];
...
/**
 Checks that missing values in the cache settings do not affect the manager
 */
- (void) testConfigurableCacheNoValues {
    self.fetcher.fetchedConfig = [[WPRemoteConfig alloc] initWithData:@{
    } version:@"1.0.0"];
...
- (void) testConfigurableCacheSubsequentCalls {
    self.fetcher.fetchedConfig = [[WPRemoteConfig alloc] initWithData:@{
        @"cacheTtl" : @123456,
        @"cacheMinAge" : @234567,
    } version:@"1.0.0"];
    ...
    [self.manager read:^(WPRemoteConfig *config, NSError *error) {
        ...
        [self.manager read:^(WPRemoteConfig *config, NSError *error) {
            XCTAssertEqual(self.manager.minimumConfigAge, 2345.678);
            XCTAssertEqual(self.manager.maximumConfigAge, 1234.567);

            // Create a new manager, default values should be the overridden values as well
            WPRemoteConfigManager *manager = [[WPRemoteConfigManager alloc] initWithRemoteConfigFetcher:self.fetcher storage:self.storage];
            XCTAssertEqual(manager.minimumConfigAge, 2345.678);
            XCTAssertEqual(manager.maximumConfigAge, 1234.567);
