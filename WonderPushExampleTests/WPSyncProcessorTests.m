...
- (void)testClassifyResponse {
    ...
    [self assertClassifyPath:@"/installation" method:@"POST" expects:@{@"mode": @"opportunistic"}];
    [self assertClassifyPath:@"/installation" method:@"PUT" expects:@{@"mode": @"opportunistic"}];
    ...
    // /user is the opportunistic resource, not the sync endpoint — GET (no /sync) is still `none`.
    [self assertClassifyPath:@"/user" method:@"GET" expects:@{@"mode": @"none"}];
